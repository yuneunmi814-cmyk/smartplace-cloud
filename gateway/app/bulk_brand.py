...
Usage:
    ...
    GATEWAY_MOCK=0 GATEWAY_HEADLESS=false \\
        python -m app.bulk_brand <네이버아이디> <brandSeq> <이미지경로> --only <placeSeq>

    # 전체 실행 (헤드리스):
    GATEWAY_MOCK=0 python -m app.bulk_brand <네이버아이디> <brandSeq> <이미지경로>

    # 처음 N곳만:
    GATEWAY_MOCK=0 python -m app.bulk_brand <네이버아이디> <brandSeq> <이미지경로> --limit 5
"""
