...
네이티브 앱 창에서 **네이버 로그인 → 지점 자동 불러오기 → 이미지/메뉴 일괄 등록 → 통계 수집**까지.
고객 PC에서 고객 네이버로 돌아가므로(Local-First) 자격증명·세션이 PC를 떠나지 않습니다.
특정 브랜드 전용이 아니라 **브랜드 번호(brandSeq)만 넣으면 어느 프랜차이즈든** 동작합니다.
...
2. **지점 불러오기** — 브랜드 번호(brandSeq) 입력 → 전 지점 자동 스크랩 (브랜드 관리 페이지 URL의 `brandSeq=` 값)
